...
def run(protocol: protocol_api.ProtocolContext):
    ...
    def bead_mixing(pipette, well, reps, vol, speed=0.5):
        ...
        side_touch_w_blowout(pipette, well, pos=-1)
        pipette.drop_tip()
    ...
    def remove_sup(well,volume1,volume2,waste_well):       
        ...
        p1000m.air_gap(20)
        p1000m.aspirate(volume1, well.bottom(4), rate=0.1)        
        p1000m.aspirate(volume2, well.bottom(clearance_bead_pellet), rate=0.02)
    ...
    def wash_mixing(pipette, well, volume,reps):
        if pipette == p1000m:
    ...
    if not skip_mixbeadsandrna:
        ...
        for index, column in enumerate(sample_wells):
            bead_mixing(pipette=p1000m, well=column[0], reps=10, vol=80, speed=0.5)
    ...
    if not skip_supremoval:
        for index, column in enumerate(sample_wells):
            remove_sup(well=column[0], volume1=75,volume2=75,waste_well=-1)
            p1000m.air_gap(20)
            p1000m.drop_tip()
    ...
    if not skip_washes:
        ...
        for repeat in range(2):
            ...
            for index, column in enumerate(sample_wells):
                ...
                p1000m.air_gap(10)
                p1000m.aspirate(180, source.bottom(clearance_reservoir)) # ht instead of 2 if using
                p1000m.dispense(190, column[0].top(), rate = 0.8)
            ...
            for column in sample_wells:
                remove_sup(well=column[0],volume1=130,volume2=50, waste_well=repeat)
                p1000m.air_gap(20)
                p1000m.drop_tip()
    ...
    if not skip_1stelution:
        ...
        for index, column in enumerate(sample_wells):
            ...
            wash_mixing(p50m, well=column[0], volume=43,reps=8)
    ...
    if not skip_rebinding:
        ...
        for index, column in enumerate(sample_wells):
            ...
            # slow mixing
            bead_mixing(pipette=p1000m, well=column[0], reps=10, vol=80, speed=0.2)
